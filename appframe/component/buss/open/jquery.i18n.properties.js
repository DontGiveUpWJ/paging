 ...
 * });
  ...
  $.i18n.properties = function (settings) {
	...
	if(settings.language.indexOf("_") != -1){
    ...
    var languagesFileLoadedCallback = function (languages) {
      ...
      for (var k = 0, m = files.length; k < m; k++) {
        ...
        // 3. with language code and country code (eg, Messages_pt_BR.properties)
		if(settings.language.indexOf("_") != -1){//luolin  四川CRM6.5专用
			if (settings.language.length >= 5) {
			  var longCode = settings.language.substring(0, 5);
			  if (languages.length == 0 || $.inArray(longCode, languages) != -1) {
			...
			if (settings.language.length >= 5) {
			  if (languages.length == 0 ) {
				loadAndParseFile(settings.path + files[k] + '_' + settings.language + settings.suffix, settings);
			  }
			}
		}
      }
  ...
  function loadAndParseFile(filename, settings) {
    ...
    //var url=filename+"?v="+new Date().getHours();
    $.ajax({
      url: filename,
      async: settings.async,
...
/**
 *  luolin modify  四川CRM6.5专用
 *  加载配置文件
 */
 $(function() {
//var __basePath = '/testIE6/';
var settings_sc = {};
	settings_sc.name = "webapp";
	settings_sc.path =__basePath+ "pub-ui/public/config/";
	settings_sc.cache = true;
	settings_sc.mode="map";
	settings_sc.language = "config";
	settings_sc.suffix='.js';
	$.i18n.properties(settings_sc);
});