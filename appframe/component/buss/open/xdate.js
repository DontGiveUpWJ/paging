...
String.prototype.toDate = function(style) {
	...
	for ( var k in compare) {
		if (new RegExp('(' + k + ')').test(style)) {
			result[compare[k]] = this.substring(tmp.indexOf(RegExp.$1), tmp
					.indexOf(RegExp.$1)
					+ RegExp.$1.length);
		}
	}
...
XDate.parsenormal=function(dateStr,style) {
	return new XDate(dateStr.toDate(style));
	//console.log(new Date(result['y'], result['M'] - 1, result['d'], result['h'], result['m'], result['s']))
	//return new XDate(date).setFullYear(result['y']).setMonth(result['M'] - 1).setDate(result['d']).setHours(result['h']).setMinutes(result['m']).setSeconds(result['s']);
};
